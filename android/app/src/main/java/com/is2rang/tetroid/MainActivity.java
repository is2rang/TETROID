package com.example.tetrio.app; // 본인의 패키지명으로 반드시 유지하세요!
...
public class MainActivity extends BridgeActivity {
    ...
    private void injectHoverController(WebView view) {
        String javascriptCode = 
            ...
            "    cacheLayout(); setTimeout(cacheLayout, 1000);" +
            ...
            "  " +
            "  // 처음 가동" +
            "  if (document.body) setupController();" +
            "  " +
            "  // 2. [좀비 모드 부활 핵심 센서] TETR.IO가 HTML을 새로 지워버리는지 감시" +
            "  if (!window.hasTetrioObserver) {" +
            "    window.hasTetrioObserver = true;" +
            "    const observer = new MutationObserver((mutations) => {" +
            "      // 우리 버튼 오버레이가 사라졌다면 다시 생성시킴" +
            "      if (!document.getElementById('touch-overlay') && document.body) {" +
            "        setupController();" +
            "      }" +
            "    });" +
            "    // body 태그 내부의 구조가 추가되거나 완전히 지워지는 현상을 상시 감시" +
